...
const forbidden = [
  /\/(?:Users|home|root|srv)\/(?!example-user(?:\/|\s|$)|example-project(?:\/|\s|$))[^\s`]+/i,
  /\/opt\/(?:private|secure)[^\s`]*/i,
  /https?:\/\/[^/\s`"']+\.(?:internal|lan)(?::\d+)?/i,
  /\b(?:owner|private)[_-](?:client|customer|host|project|tenant|user)[_-][A-Za-z0-9._-]+\b/i,
  /\bPUBLIC_SYNC\.md\b/i,
...
for (const fixture of [
  "/Users/public-fixture-user/project",
  "/srv/fixture-project",
  "/opt/private-fixture/config",
  "private_client_fixture"
]) {
  assert(
    forbidden.some((pattern) => pattern.test(fixture)),
    `public boundary pattern did not reject neutral fixture: ${fixture}`
  );
}
...
for (const path of ["scripts/smoke-agent-observability.mjs", "scripts/smoke-memory-keeper.mjs"]) {
  const text = await read(path);
  mustNotMatch(text, [/-----BEGIN [A-Z ]*PRIVATE KEY-----/i], path);
}
...
const forbiddenOwnerCodeMarkers = [
  ...
  /\/opt\/(?:private|secure)[^\s`"']*/i,
  /https?:\/\/[^/\s`"']+\.(?:internal|lan)(?::\d+)?/i,
  /\bconst\s+\w*default\w*server\w*url\s*=\s*["']https?:\/\/[^"']+["']/i
];
