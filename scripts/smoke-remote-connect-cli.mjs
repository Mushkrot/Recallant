      ...
      if (body.method === "tools/call") {
        ...
        if (toolName === "memory_get_readiness_status") {
          ...
          structuredContent = {
            ...
            session_id: args.session_id ?? "00000000-0000-4000-8000-000000000011"
          };
...
const onboardRemoteServer = await connectServer("approved-after-pending");
...
const onboardRemoteProject = await mkdtemp(join(tmpdir(), "recallant-onboard-universal-"));
const universalHome = await mkdtemp(join(tmpdir(), "recallant-connect-universal-home-"));
const onboardRemoteHome = await mkdtemp(join(tmpdir(), "recallant-onboard-universal-home-"));
try {
  const onboardRemote = await runCli(
    [
      "onboard",
      onboardRemoteProject,
      "--poll-timeout-ms",
      "5000",
      "--poll-interval-ms",
      "50",
    ...
    {
      env: {
        HOME: onboardRemoteHome,
        RECALLANT_DATABASE_URL: "",
        RECALLANT_ENV_FILE: join(onboardRemoteHome, "missing.env"),
        RECALLANT_CONNECT_SERVER_URL: onboardRemoteServer.baseUrl,
  ...
  assert(
    onboardRemote.status === 0,
    `universal onboard failed: ${onboardRemote.stderr}; starts=${JSON.stringify(onboardRemoteServer.state.startBodies)}`
  );
  const onboardRemoteJson = JSON.parse(onboardRemote.stdout);
  assert(
    onboardRemoteJson.action === "connect_cloud" && onboardRemoteJson.status === "connected",
    `onboard did not route through universal remote connect: ${onboardRemote.stdout}`
  );
  assert(
    onboardRemoteServer.state.startBodies.length === 1 &&
      (await exists(join(onboardRemoteProject, ".codex", "config.toml"))),
    "universal onboard did not complete remote project setup"
  );
  ...
  try {
    await mkdir(join(connectRemoteTextProject, ".recallant"), { recursive: true });
    await writeFile(
      join(connectRemoteTextProject, ".recallant", "remote-consent.json"),
      JSON.stringify({
        kind: "recallant_remote_agent_consent",
        consent_scope: {
          destination: { server_url: "https://old.example.com", endpoint_path: "/api/mcp" },
          credential_scope: {
            project_id: "00000000-0000-4000-8000-000000000015",
            developer_id: "00000000-0000-4000-8000-000000000010",
            client_id: "stale-client",
            credential_prefix: "rcl_stale"
          },
          allowed_context: [],
          redaction_boundary: [],
          not_sent: [],
    ...
    const refreshedConsentReceipt = JSON.parse(
      await readFile(join(connectRemoteTextProject, ".recallant", "remote-consent.json"), "utf8")
    );
    assert(
      refreshedConsentReceipt.consent_scope?.credential_scope?.project_id ===
        "11111111-1111-4111-8111-111111111111" &&
        refreshedConsentReceipt.consent_scope?.credential_scope?.client_id ===
          "remote-cli-smoke-text",
      "connect-remote did not refresh a stale remote consent receipt"
    );
  } finally {
...
try {
  ...
  await closeServer(pendingServer.server);
