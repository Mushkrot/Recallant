...
async function writeExistingProjectFixture(projectDir) {
  ...
  await writeFile(
    ...
    [
      "OPENAI_API_KEY=example-password",
      `GITHUB_TOKEN=${githubTokenFixture}`,
      "DATABASE_URL=postgres://example-user:example-password@127.0.0.1:5432/example-db",
      ""
    ].join("\n")
  );
}
...
assert(!agents.includes(openAiProviderTokenFixture), "AGENTS.md still contains raw legacy token");
