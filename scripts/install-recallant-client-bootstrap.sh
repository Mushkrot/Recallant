...
valid_remote_url() {
  local value="$1"
  [[ "$value" =~ ^https://[^[:space:]/?#]+([^[:space:]]*)?$ || "$value" =~ ^http://127\.0\.0\.1(:[0-9]+)?(/[^[:space:]]*)?$ ]]
}
...
if [[ "$CONNECT_MODE" == "true" ]]; then
  valid_remote_url "$CONNECT_URL" || fail "Remote connect requires an HTTPS Recallant server URL." 2
elif [[ "$INVITE_MODE" == "true" ]]; then
  valid_remote_url "$INVITE_URL" || fail "Remote invite requires an HTTPS Recallant server URL." 2
else
  valid_remote_url "$SERVER_URL" || fail "Remote onboarding requires an HTTPS Recallant server URL from the central server package." 2
fi
...
if ! valid_remote_url "$SERVER_URL"; then
