...
async function runConnectRemote(env, fixture) {
  ...
  const recallantEntries = await readdir(join(projectDir, ".recallant"));
  assert(
    recallantEntries.length === 1 && recallantEntries[0] === "remote-consent.json",
    "remote client path created .recallant storage beyond the non-secret consent receipt"
  );
  const remoteConsentReceiptText = await readFile(
    join(projectDir, ".recallant", "remote-consent.json"),
    "utf8"
  );
  assertNoLeak("remote-consent-receipt", remoteConsentReceiptText);
  const remoteConsentReceipt = JSON.parse(remoteConsentReceiptText);
  assert(
    remoteConsentReceipt.kind === "recallant_remote_agent_consent",
    "remote consent receipt missing expected kind"
  );
  assert(
    remoteConsentReceipt.no_raw_credentials_or_private_keys === true,
    "remote consent receipt did not declare its redaction boundary"
  );
  assert(
    remoteConsentReceipt.consent_scope?.credential_scope?.project_id === expectedScope.projectId &&
      remoteConsentReceipt.consent_scope?.credential_scope?.developer_id ===
        expectedScope.developerId &&
      remoteConsentReceipt.consent_scope?.credential_scope?.client_id === expectedScope.clientId,
    "remote consent receipt did not retain the expected scoped identity"
  );
  assert(
  ...
  return {
    ...
    no_local_storage_beyond_remote_consent_receipt:
      recallantEntries.length === 1 && recallantEntries[0] === "remote-consent.json",
