...
function staticClientBootstrapResult(args, env = {}) {
  ...
  assert(
    clientBootstrapSource.indexOf('valid_remote_url "$SERVER_URL" || fail') <
      clientBootstrapSource.indexOf('if [[ -n "${RECALLANT_CLIENT_BOOTSTRAP_RECALLANT_CMD:-}" ]]'),
...
for (const [label, source] of [
  ["development reset", devMakefileSource],
  ["development healthcheck", devComposeYaml],
  ["production install", installerSource],
  ["production healthcheck", prodComposeYaml]
]) {
  assert(
    source.includes("psql") && source.includes("SELECT 1") && !source.includes("pg_isready"),
    `${label} must wait for a successful query in the configured database`
  );
}
...
assert(
  installerSource.indexOf("docker info >/dev/null 2>&1") <
    ...
    bootstrapSource.indexOf("docker info >/dev/null 2>&1") <
      bootstrapSource.indexOf('if [[ -e "$INSTALL_DIR" && ! -d "$INSTALL_DIR/.git" ]]'),
  "Installers must fail early with a human Docker-not-running message"
);
assert(
  bootstrapSource.includes("--install-dir <path>") &&
    bootstrapSource.includes('INSTALL_DIR="${RECALLANT_BOOTSTRAP_INSTALL_DIR:-}"') &&
    bootstrapSource.includes('git -C "$INSTALL_DIR" fetch --depth 1 origin "$SOURCE_REF"') &&
    bootstrapSource.includes('"refs/tags/$SOURCE_REF:refs/tags/$SOURCE_REF"') &&
...
for (const [index, badUrl] of ["not-a-url", "http://recallant.example.com"].entries()) {
  const badUrlResult = runClientBootstrap(
    ...
    {
      RECALLANT_CLIENT_BOOTSTRAP_INSTALL_DIR: join(
        clientBootstrapFixtureRoot,
        `bad-url-install-${index}`
      )
    }
  );
