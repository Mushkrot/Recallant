...
async function run() {
  ...
  const reportDir =
    process.env.RECALLANT_PLAYWRIGHT_REPORT_DIR ?? join(tmpdir(), "recallant-playwright-reports");
  const publicReportDir = join(reportDir, "public-safe-candidates");
  const portfolioScreenshotDir = process.env.RECALLANT_PORTFOLIO_SCREENSHOT_DIR;
  const portfolioScreenshots = portfolioScreenshotDir
    ? {
        home: join(portfolioScreenshotDir, "home-readiness.png"),
        review: join(portfolioScreenshotDir, "review-provenance.png"),
        activity: join(portfolioScreenshotDir, "activity-recovery.png")
      }
    : {};
  ...
  if (portfolioScreenshotDir) await mkdir(portfolioScreenshotDir, { recursive: true });
  ...
  assert(
    automaticRecoveryChain?.status === "verified" &&
      ...
      automaticRecoveryChain.steps.some((step) => step.stage === "verification" && step.automatic),
  ...
  try {
    ...
    await savePortfolioScreenshot(
      desktop,
      portfolioScreenshots.home,
      "portfolio Home",
      forbiddenPublicText
    );
    ...
    await savePortfolioScreenshot(
      desktop,
      portfolioScreenshots.activity,
      "portfolio Activity errors",
      forbiddenPublicText
    );
    await desktop.screenshot({
    ...
    const portfolioReviewSource = desktop
      .locator("p.source-note:visible", {
        hasText: "From source Archived design-review notes kept only as provenance"
      })
      .first();
    await portfolioReviewSource.waitFor();
    await savePortfolioScreenshot(
      desktop,
      portfolioScreenshots.review,
      ...
      JSON.stringify(
        {
          ...
          portfolio_screenshots: Object.values(portfolioScreenshots),
          ...
          checks: [
            ...
            ...(portfolioScreenshotDir ? ["portfolio_screenshot_set"] : []),
