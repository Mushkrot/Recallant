...
RECALLANT_EXPECTED_OLLAMA_MODELS=nomic-embed-text,mistral-small3.2:24b
RECALLANT_MANAGEMENT_CHAT_AI=on
RECALLANT_MANAGEMENT_CHAT_MODEL=mistral-small3.2:24b
...
prod_compose exec -T postgres sh -c 'until psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -tAc "SELECT 1" >/dev/null 2>&1; do sleep 1; done'
