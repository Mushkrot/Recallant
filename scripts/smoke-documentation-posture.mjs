...
assert(
  !serializedProduction.includes("example-password"),
...
assert(
  productionAiInput.length <= 6_000 && !productionAiInput.includes("example-password"),
  ...
  `${JSON.stringify(
    {
      ...
      starter_docs: {
        ...
        private_marker_free: [starterUnknown, starterService, starterProduct, starterLibrary].every(
          (plan) => {
            const serialized = JSON.stringify(plan);
            return [`/${"ai"}/`, `${"uni"}${"cloud"}`, `${"example"}-${"service"}`].every(
      ...
      ai: {
        ...
        input_sample: {
          ...
          leaked_fixture_secret: productionAiInput.includes("example-password")
        },
        high_confidence: {
