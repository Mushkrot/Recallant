...
assert(
  missingPayload.storage?.setup_choices?.some(
    ...
    missingPayload.storage?.remote_connect?.available === true &&
    missingPayload.storage?.remote_connect?.server_url === null &&
    missingPayload.storage?.remote_connect?.command === null,
  `missing storage did not offer remote central-server connect: ${JSON.stringify(missingPayload.storage)}`
);
assert(
  !JSON.stringify(missingPayload).includes("example-password") &&
    !JSON.stringify(missingPayload).includes(databaseUrl),
  "missing storage output leaked database credentials"
);
const missingStorageText = runRaw(recallant, ["onboard", missingStorageProject, "--local"], {
  cwd: missingStorageProject,
  omitDatabaseUrl: true,
  env: {
    RECALLANT_ENV_FILE: join(missingStorageProject, "missing-recallant.env")
...
assert(
  (missingStorageText.stdout.match(/^Next action:/gm) ?? []).length === 1 &&
    ...
    missingStorageText.stdout.includes("curl -fsSL <server>/connect | bash") &&
...
const unconfiguredRemoteOnboard = runRaw(
  recallant,
...
assert(
  unconfiguredRemoteOnboard.status === 2,
  `unconfigured remote onboard should exit 2: ${unconfiguredRemoteOnboard.stderr}\n${unconfiguredRemoteOnboard.stdout}`
);
const unconfiguredRemotePayload = JSON.parse(unconfiguredRemoteOnboard.stdout);
assert(
  unconfiguredRemotePayload.action === "onboard" &&
    unconfiguredRemotePayload.status === "storage_blocked" &&
    unconfiguredRemotePayload.storage?.remote_connect?.server_url === null &&
    unconfiguredRemotePayload.storage?.remote_connect?.command === null,
  `unconfigured remote onboard should stop before selecting a server: ${JSON.stringify(unconfiguredRemotePayload)}`
);
...
assert(
  !JSON.stringify(defaultEnvOnboard).includes("example-password") &&
...
assert(
  !JSON.stringify(explicitEnvOnboard).includes("example-password") &&
...
assert(
  !JSON.stringify(wrapperEnvOnboard).includes("example-password") &&
...
for (const marker of ["old handoff", "example-password", "raw old handoff"]) {
  assert(
...
await writeFile(
  ...
  "# Existing Agent Notes\n\nKeep deployment notes under review. API_SECRET=example-password\n"
);
await writeFile(join(productionProject, ".env.example"), "API_SECRET=example-password\n");
...
assert(
  !JSON.stringify(productionYes).includes("example-password") &&
...
assert(
  !JSON.stringify(storedPosture).includes("example-password") &&
...
assert(
  !JSON.stringify(contextPack).includes("example-password") &&
  ...
  `${JSON.stringify(
    {
      ...
      stored_posture_setting_excerpt: {
        ...
        raw_secret_leaked: JSON.stringify(storedPosture).includes("example-password")
      },
      ...
      context_pack_canon_capability_excerpt: {
        ...
        bounded_max: 8,
        raw_secret_leaked: JSON.stringify(contextCanon).includes("example-password")
      },
