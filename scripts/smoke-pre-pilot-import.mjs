...
if (JSON.stringify(envPreview).includes("example-password")) {
...
if (
  ...
  envImport.migration_memory_candidate?.source_refs?.[0]?.metadata?.source_path !==
    ".env.example" ||
  JSON.stringify(envImport).includes("example-password") ||
  JSON.stringify(envImport).includes("example-password")
) {
...
try {
  ...
  if (
    String(row.env_chunk_text).includes("example-password") ||
    String(row.env_chunk_text).includes("example-password")
  ) {
