...
- Do not commit secrets, raw credentials, private server paths, raw memory exports, customer data, or
  local runtime state.
- For ordinary user-facing project connection guidance, lead with the universal `recallant onboard
  <project>` or `recallant onboard .` command. It uses reachable local storage or central remote
  storage supplied through project configuration, deployment environment, or `--server-url`; it
  must not embed a maintainer deployment hostname as a public default. Treat `recallant connect`,
  `recallant attach`, and client-specific `recallant connect <client> --project-dir ...` as
  lower-level/debug paths unless the user explicitly asks for those internals.
