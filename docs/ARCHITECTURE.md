...
- **Project continuity pack:** a bounded context view designed to let one real agent client resume
  work recorded by another.
- **Document source:** an explicitly permitted file or document set whose identity and lifecycle
  stay distinct from derived chunks and memories.
- **Task handoff record:** a planned Recallant-owned work-state object linked to memory and evidence;
  it is not an external issue-tracker record.

The canonical product vocabulary is maintained in [CONTEXT.md](../CONTEXT.md).

## Product Layers And Authority

Recallant owns the authoritative Project Memory: project identity, evidence, indexes, provenance,
governed memories, lifecycle, review, and policy. Agent clients and human interfaces are consumers
and contributors within that boundary; no model client owns the memory.

The product layers are deliberately ordered:

1. The existing governed memory core and Codex lifecycle.
2. Cross-Client Continuity between real Codex and Claude Code sessions.
3. Human Document Memory over explicitly attached sources.
4. A Task Handoff Record layered over proven continuity and evidence.
5. Broader human sources and connectors.

Linear is not a required component. Plane or another task manager may later be connected as an
optional External Task Adapter or board, but it must not become a second authority for Project
Memory or a required runtime dependency.

## Project Bootstrap

`recallant onboard <project>` is the product-level path for making a project agent-ready. It routes
to local onboarding when storage is reachable on the same host, or to the remote existing-server
path when a server URL is supplied through project configuration, deployment environment, or
`--server-url`. With neither route configured, it stops before project writes. It owns storage
readiness, project attach or scoped remote credential setup, client connection, capture proof,
recall proof, and Workbench visibility. Under the local path it creates or updates thin local
pointers such as `.recallant/config`, `AGENTS.md`,
...
Before storage selection in an interactive terminal, onboarding performs a fail-open CLI revision
check against the official public `main` branch. A confirmed self-update is accepted only from the
official HTTPS repository and only when the installation checkout is clean. The updated executable
then resumes the original onboarding invocation. Non-interactive, JSON, offline, explicitly skipped,
and custom-branch runs continue without automatic mutation.
...
The shipped graph layer remains supported, but further graph depth is not an active product
milestone unless Cross-Client Continuity or Human Document Memory demonstrates a concrete retrieval
or governance need for it.

## Planned Human Document Memory

After Cross-Client Continuity v1, Recallant plans to serve both AI agents and people through
AI-mediated question answering over explicitly permitted documents. This roadmap decision does not
mean broad filesystem crawling or passive ingestion exists today.

The future architecture should preserve the existing governance model:

- original artifacts remain distinct from extracted evidence, derived candidates, accepted memory,
  and instruction-grade rules;
- Recallant remains authoritative for source identity, indexes, provenance, lifecycle, policy, and
  governed memory, while original systems remain authoritative for their source bytes and versions;
- human and agent clients use one governed query-and-answer layer rather than storage-specific
  retrieval behavior;
- Obsidian is an optional source, capture surface, or generated projection, not a peer authoritative
  database or a path around review;
- the first MVP is limited to explicitly attached Markdown, text, PDF, and DOCX sources with
  incremental add/update/delete semantics and source-cited answers;
- broader local-file indexing, passive synchronization, media, and connectors remain later,
  separately governed capabilities.

The evidence and trust boundaries are recorded in the
[Jarvis and Obsidian research note](research/JARVIS_OBSIDIAN_COMPARISON.md#accepted-future-direction).

## Planned Task Handoff Layer

The Task Handoff Record is a future layer above Project Memory, not a replacement for it. It will
carry objective, status, actor, boundaries, acceptance checks, source refs, evidence, checkpoint,
blockers, next action, and receipt state. Open Engine is a process reference for this layer.

The record is intentionally scheduled after real cross-client continuity. A task record cannot
compensate for missing shared memory by becoming another pasted summary. Recallant will implement
only the work-state contract needed by agents and people; it will not build a full Linear/Jira
replacement.
