...
The 2026-08-01 strategic refresh assigns each major reference a bounded role:

- AgentMemory: primary native Codex/Claude Code adapter reference;
- MemPalace: primary document-ingestion and secondary adapter reference;
- Open Brain / OB1: memory-substrate and trust-model reference;
...
approved dependencies or code-copy decisions.

## Open Brain / OB1

Snapshot date: 2026-08-01. Refresh before major architecture decisions.
...
The current repository remains a broad toolkit of schemas, recipes, integrations, dashboards, and
guidance rather than one cohesive replacement product. Its FSL-1.1-MIT license also restricts
competing use of each version before that version later converts to MIT, so code reuse requires a
...
Workbench review, and audit. The task-handoff lesson remains valuable, but implementation follows
real Cross-Client Continuity and the Human Document Memory MVP rather than preceding them.
...
- Optimize for tool plurality. Claude, Codex, ChatGPT, browser agents, local agents, task-system
  adapters, and humans can all be part of one job without crowning one client as the only interface.
...
- Do not make Linear or another hosted task service a required dependency. Recallant owns the Task
  Handoff Record; external boards may only be optional adapters.
...
- If a human board later becomes useful, evaluate Plane or another self-hosted interface as an
  optional adapter. Do not fork a full project manager or create a second memory authority merely to
  obtain a board.
...
- Current repository: [makeplane/plane](https://github.com/makeplane/plane)
- Community Edition overview: [Plane Open Source](https://plane.so/open-source)

### Role For Recallant

Plane is the closest current self-hosted open-source product reference for a Linear-like human task
interface. Its AGPL-3.0 Community Edition includes projects, work items, cycles, modules, pages,
multiple layouts, REST API, and webhooks.

Plane is not a Recallant foundation or an active dependency. Evaluate it only after the
Recallant-owned Task Handoff Record exists and only if a real human workflow needs a richer board
than Workbench should provide. Prefer an upstream-compatible adapter over a fork. Keep Project
Memory, task evidence, governance, and authoritative handoff state in Recallant, and review AGPL and
commercial-edition boundaries before integration or code reuse.
...
MemPalace is the strongest current implementation reference for Recallant's combined client-adapter
and document-ingestion path. The inspected project includes Claude Code, Codex, and Cursor
integrations; project and conversation mining; optional PDF, DOCX, PPTX, XLSX, EPUB, and RTF
extraction; and Chroma, SQLite, Milvus, Qdrant, and pgvector storage options. Its center of gravity
remains verbatim-first capture and retrieval rather than Recallant-style governed promotion.
...
- Reuse document extraction, source update, backend abstraction, and client-hook patterns at the
  architecture level before inventing parallel mechanisms.
...
- Do not flatten raw documents, extracted chunks, generated synthesis, and accepted memory into one
  verbatim archive.

## AgentMemory

Snapshot date: 2026-08-01. Refresh before major client-integration decisions.
...
AgentMemory is the strongest current reference for proving that real coding clients are connected
and recording. The inspected project has separate Claude Code and Codex plugins with native hooks,
MCP, a shared server, a viewer, provenance/audit, hybrid search, and sandboxed retrieval evaluation.
...
- Separate client packages that share one server contract instead of forcing every client through a
  Codex-shaped adapter.
...
Snapshot date: 2026-08-01. Refresh before major memory-product decisions.
...
- [Archived mem0-mcp repository](https://github.com/mem0ai/mem0-mcp)
...
The former standalone `mem0-mcp` repository was archived in March 2026 in favor of Mem0's hosted MCP
path, which reinforces the need to verify local/self-hosted boundaries rather than treating the
OpenMemory name as one stable implementation.
...
- Current repository: [odysseus-dev/odysseus](https://github.com/odysseus-dev/odysseus)
- Current license file:
  [AGPL-3.0 License](https://raw.githubusercontent.com/odysseus-dev/odysseus/dev/LICENSE)
- Current README:
  [Odysseus](https://raw.githubusercontent.com/odysseus-dev/odysseus/dev/README.md)
- Public landing page:
  [Odysseus - A Self-Hosted AI Workspace](https://odysseus-dev.github.io/odysseus/)
- Releases page:
  [GitHub releases](https://github.com/odysseus-dev/odysseus/releases)
...
- The project moved from the earlier personal namespace into the `odysseus-dev` organization.
- The current top-level license is AGPL-3.0, replacing the permissive-license assumption in the
  earlier snapshot. Treat it as a product/UX reference unless a deliberate license review approves
  code reuse.
- Its scope remains a broad human AI workspace rather than an external memory substrate for existing
...
- Do not reuse code without AGPL, dependency, and third-party-license review.
...
Current README and strategic role rechecked 2026-08-01. The release/traction observations below
remain dated 2026-07-02.
...
local Memory Tree / Obsidian-style vault, SuperContext first-turn preparation, documents, goals and
todos, integrations, MCP/skills catalogs, native tools, model routing, voice, meetings, and optional
local AI. Its current README also documents an optional AgentMemory backend intended to share memory
with coding clients including Claude Code, Cursor, Codex, and OpenCode.
...
- The optional AgentMemory backend is a useful integration reference for presenting one human
  workspace over a memory service also used by external coding agents.
...
- First-party system description:
  [How to Build a Voice-Driven Agentic OS on Claude Code](https://www.chaseai.io/blog/voice-driven-agentic-os-claude-code)
- Underlying architecture pattern:
  [Andrej Karpathy's LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
- Focused Recallant comparison:
  [Jarvis, Obsidian, and the LLM Wiki pattern](JARVIS_OBSIDIAN_COMPARISON.md)

### Current Read

Chase AI's Jarvis is a product and workflow reference, not a verified reusable memory engine. The
first-party description matches the distinctive stack shown in public summaries: a web interface
over Claude Code skills, Obsidian-backed reports, Faster Whisper for local speech-to-text, and
Kokoro for local text-to-speech. For Recallant, the relevant part is not voice. It is the use of an
Obsidian vault as a human-readable store for generated reports and the decision to read an existing
report before asking headless Claude Code to generate a missing one.

The memory idea is better attributed to Karpathy's LLM Wiki pattern. That pattern has three layers:
immutable raw sources, an LLM-maintained Markdown wiki, and a schema file such as `CLAUDE.md` or
`AGENTS.md`. It also proposes an `index.md`, an append-only `log.md`, citations, contradiction
tracking, periodic linting, optional local search, and Git history. Obsidian is the human-facing IDE
for the files; it is not the reasoning engine or a required database service.

Important evidence limits:

- The inspected first-party Jarvis article does not link to a public Jarvis source repository,
  memory schema, retrieval evaluation, or reproducible benchmark. Treat its behavior as an author
  description, not code-verified infrastructure.
- The article says Jarvis reads and generates reports in Obsidian. It does not substantiate the
  stronger claim that every answer is automatically saved. Karpathy's source says useful answers
  _can_ be filed back into the wiki.
- A plain LLM Wiki can operate without a database service at modest scale, but that does not prove
  the whole Jarvis system is database-free. The same Jarvis description names a separate LightRAG
  skill, and Karpathy recommends optional indexed search when the wiki grows.
- Local speech components do not make the whole system local. The described reasoning path can use
  Anthropic models and headless Claude Code.

Broad local-file indexing remains a separately governed capability. This reference does not by
itself authorize filesystem crawling or data access. The accepted Human Document Memory milestone
and its explicit MVP boundary are recorded in the
[focused Jarvis and Obsidian comparison](JARVIS_OBSIDIAN_COMPARISON.md#accepted-future-direction).

### What Is Strong For Recallant

Human ownership and portability:

- Markdown is immediately inspectable, editable, diffable, exportable, and recoverable without a
  special Workbench or database client.
- Obsidian gives humans fast navigation, backlinks, graph view, and a familiar editing surface
  while leaving the underlying files tool-independent.
- Git can provide understandable version history, branching, and rollback for generated knowledge.

Knowledge compilation:

- Separating immutable raw sources from generated synthesis is a clean trust boundary.
- Incrementally maintaining entity, topic, comparison, and synthesis pages can avoid rebuilding the
  same multi-source understanding for every question.
- A schema file makes structure and maintenance rules explicit instead of hiding them in agent
  prompts.
- `index.md`, `log.md`, citations, contradiction checks, orphan detection, and periodic linting are
  simple mechanisms that remain useful even before embeddings or a graph database are justified.

### What Not To Copy

- Do not make agent-edited Markdown automatically instruction-grade. A readable file can still be
  stale, hallucinated, contradictory, or written under the wrong project or authority.
- Do not save every answer as durable memory. Preserve useful conclusions selectively and keep
  source evidence separate from generated synthesis.
- Do not treat wikilinks or a visually convincing graph as proof of provenance, lifecycle, review,
  scope, or retrieval quality.
- Do not replace Recallant's project/developer/audience isolation, review states, audit, retention,
  erasure, and secret policy with folder conventions enforced only by prompts.
- Do not couple the memory contract to Claude Code. The vault pattern is model-agnostic; Recallant's
  MCP and server contracts should remain multi-client.
- Do not claim faster or more accurate recall without a representative retrieval evaluation. The
  inspected Jarvis material publishes no such measurement.

### How Recallant Can Use This Reference

Obsidian companion-layer checklist:

- Keep governed Postgres memory as the authoritative state, while treating an Obsidian-compatible
  vault as an optional human-readable projection and editing surface.
- Build on the existing dry-run-first `recallant vault inventory`, `vault candidates`, and
  `vault export` contract instead of introducing an unrelated sync path.
- Export accepted, source-linked decisions, checkpoints, open questions, and memory-review items;
  keep raw evidence and generated synthesis visibly distinct.
- Route human or agent edits from the vault back through source-linked candidates and review rather
  than silently overwriting accepted memory.
- Consider index, append-only change log, backlinks, and vault linting as incremental export
  improvements, then measure whether they improve real recall and review workflows.
- Do not ship passive two-way sync until ownership, conflicts, deduplication, deletion, secret
  handling, and rollback semantics are explicit and tested.
