...
**Internal architecture shorthand:** AMP / Agent Memory Platform  
...
AMP is intended to become a **full-quality working product for the owner's real AI-assisted development workflow**, not a quick prototype. The project may take more design and implementation effort if that is required for a coherent core.

Current architectural bias:

- **Open Brain / OB1** is the preferred foundation.
- Accepted synthesis: combine OB1 governance with MF0 workbench/raw-capture/Memory Tree/Keeper ideas; AMP owns the bridge through managed hybrid capture profiles, Review UI, context-budget policy, and local-server-first deployment.
...
- Review UI runs on the AMP server; v1 starts as a compact private workbench and should evolve into a broader management platform.
...
- Settings are centralized on the AMP server; project repositories store only pointer config.
...
- Security/access posture is private-by-default plus AMP auth: localhost/Tailnet/SSH by default, Review UI/admin API require AMP auth even inside private network, and Cloudflare-managed access is a near-future opt-in mode requiring edge auth plus AMP auth.
...
Новый проект должен подключаться к AMP без ручного копирования всей структуры правил и логов.

**Измеримость:**

- [ ] `amp init --target codex` создаёт `.amp/config`, тонкий `AGENTS.md`, `PROJECT_LOG.md`, и нужный MCP/config output для Codex.
- [ ] `amp init --dry-run` показывает план без изменений.
...
Codex must work first, but AMP must not become Codex-specific.
...
AMP должен улучшать качество работы агента без загрузки огромных файлов при старте сессии.
...
Core AMP работает на личном Linux-сервере владельца, с локальными embedding/consolidation задачами и optional внешними LLM для сложного анализа.
...
- [ ] Paid API is not used silently after subscription limits are exhausted; AMP defers/downgrades/asks according to policy.
- [ ] Default paid API mode is `confirm_each`; every direct paid API request requires explicit owner approval before execution.
- [ ] AMP management UI includes a near-real-time cost dashboard for paid API estimates, approvals, providers/models, purposes, and project totals.
...
AMP должен позволять работать локально, когда сервер недоступен, интернет медленный или live MCP write path временно не работает.

**Измеримость:**

- [ ] Local spool пишет append-only JSONL/NDJSON records with dedup keys.
- [ ] `amp sync-spool` загружает локальные записи на сервер и сохраняет mapping local id → server `event_id`.
...
AMP must be restorable after server/database/artifact failure.

**Измеримость:**

- [ ] Automated backup includes `amp_agent_work` Postgres database.
...
AMP must protect memory and management surfaces by default while remaining ready for a near-future Cloudflare-managed subdomain.

**Измеримость:**

- [ ] Default deployment binds Review UI/admin API to localhost or Tailnet/private interface.
- [ ] Review UI/admin API require AMP auth/session/token even on private network.
- [ ] Postgres is not exposed publicly and is reachable only by AMP runtime or explicit trusted admin operations.
...
- [ ] Cloudflare mode requires edge auth such as Cloudflare Access or equivalent plus AMP auth.
- [ ] No unauthenticated public management, MCP, backup, or raw-artifact route exists.

### G11 — Owner review UI for governed memory

AMP v1 должен дать владельцу полноценный UI для управления важной, конфликтной и долгосрочной памятью.

Placement: the UI runs on the AMP server. It starts as a compact working review/cost/settings workbench, not a minimal approval table, while the architecture allows growth into a full private management platform.
...
13. **As the owner**, I want centralized settings so I can open AMP management UI, choose a project, and inspect or change its project-specific settings without editing local files.
14. **As the owner**, I want a cost dashboard and explicit paid API approvals so AMP cannot quietly add token bills on top of my existing agent subscriptions.
...
- Repo-native AMP contract generated by `amp init` — см. `REPO_CONTRACT.md`. The older personal `agent-bootstrap` sketch is only historical inspiration.
...
- private-by-default access with AMP auth and Cloudflare-ready routing,
