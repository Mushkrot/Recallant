...
Do not start by connecting a real project. Existing-project discovery/preflight is complete for the first pre-pilot checkpoint; the next implementation work is Durable Explicit Import write mode, followed by Review UI import handling and sandbox pilot workflow.
...
8. Begin with Workstream R2 from `docs/PRE_PILOT_READINESS.md`.

## Active Work Order

1. R1 Existing Project Discovery And Preflight. Complete for the first pre-pilot checkpoint.
2. R2 Durable Explicit Import. Start here next.
...
R0 Documentation And Handoff Readiness is complete for the current checkpoint. R1 discovery/preflight is also complete for the first pre-pilot checkpoint.
