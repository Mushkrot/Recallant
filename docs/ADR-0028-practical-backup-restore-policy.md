...
Recallant exists to prevent loss of working memory and project context. Therefore Recallant itself must not become a single fragile point of memory loss.
...
- initial deployment may keep backups on the same Recallant server,
...
Recallant v1 will implement a **practical backup/restore policy**.

The backup set must cover:

1. `recallant_agent_work` Postgres database.
...
4. Non-secret deployment/config metadata needed to restore the Recallant server.
...
- Initial backup target may be local storage on the same Recallant server.
...
- Backups should be encrypted before leaving the Recallant server.
- A backup is not considered healthy only because a file exists. Recallant must support restore verification into a temporary database/location.
...
These are future hardening options if Recallant becomes more critical or data volume requires them.
...
- The future second-server backup path is part of the architecture even if the first deployment stores backups on the Recallant server.
