...
- Remote source health now treats remote `server_path` references as governed access-binding work
  instead of resolving them as local paths. Remote repo references are marked as needing sync/import,
  connector references can become ready only when governed capability-binding metadata exists, and
  document collections show provenance-ready copy without pretending connector capture is active.
...
- Management chat policy-guard pass: local AI still performs semantic interpretation, but
  deterministic server policy now overrides obvious under-classification for cleanup, developer-wide
...
- Pilot reports now include cross-project recall evidence: after sandbox detach, active/default
  cross-project recall hides detached projects, while explicit `include_detached=true` can retrieve
  the copied sandbox as a source-linked example. The report asserts those results are not binding
  rules and remain examples until applied through governed memory.
