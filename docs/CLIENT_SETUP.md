...
The same command works on macOS, arbitrary servers, and a Recallant host. It selects reachable local
storage or a central server supplied through project configuration, deployment environment, or
`--server-url`. If neither is configured, it stops before changing project files and shows both
setup paths.
...
`recallant connect` remains available as an advanced compatibility alias:
...
The remote existing-server path must not require local Docker, Postgres, `RECALLANT_DATABASE_URL`,
...
`recallant connect .` remains an advanced compatibility alias. If the installed CLI predates the
universal `onboard` route, refresh it from the central server bootstrap or from a current Recallant
checkout before retrying.

In an interactive terminal, `recallant onboard .` checks its installed Git revision against the
official `Mushkrot/Recallant` `main` branch before changing project files. When an update is
available, the CLI displays a compact current-to-latest revision panel and asks for confirmation.
If accepted, it fetches only from the official HTTPS repository, refuses to overwrite a checkout
with local changes, installs the user-local CLI, and resumes onboarding. A failed or unavailable
update check does not block onboarding. JSON and non-interactive runs do not prompt; interactive
users can pass `--no-update-check` to skip the check.

The automatic check is channel-aware. A `main` install can offer a newer `main` revision.
The tagged prerelease stays pinned. It does not query or switch to `main` during onboarding.
Changing channels is an explicit owner action, such as rerunning the trusted bootstrap with
`--ref main`.

After the CLI is installed or refreshed, `recallant --version` should report the CLI package version
plus git build metadata, such as `recallant 0.1.0-dev.3+<git-sha>`. All monorepo workspace manifests
...
This is configuration parity, not native capture parity. Recallant does not yet install a Claude
Code lifecycle adapter comparable to `recallant codex-hook`, and the current product does not claim
a real Codex -> Claude Code -> Codex continuation proof. That adapter and acceptance journey are the
active [Cross-Client Continuity v1 roadmap milestone](ROADMAP.md#milestone-1-cross-client-continuity-v1).
...
Older client builds may not have the universal `onboard` route and can stop at local-storage setup;
refresh the remote client or use `connect-cloud` directly before retrying the beginner command.
