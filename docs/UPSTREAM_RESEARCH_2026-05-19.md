...
Internal working notes for AMP architecture discovery. This is not a user-facing comparison and does not decide the final architecture.
...
Important working rule from the project owner: licensing is not a selection criterion at this stage. We can use any listed project as needed. The practical question is whether a component fits AMP technically, operationally, and contractually.

## Working Architecture Bias

OB1 should be treated as the backbone because its architecture most closely matches the desired AMP shape:
...
- agent-bootstrap: owner's early repo-contract sketch for durable handoff files; treat as prior AMP thinking, not as a mature external upstream.
...
Architectural implication: do not keep re-running a neutral "which project wins" contest. Start from OB1 and deliberately graft in stronger subsystem patterns where they help AMP.
...
Most important new direction for AMP:
...
- The single flat `thoughts` table as AMP core. AMP needs raw L0 events, chunks, embeddings, edges, checkpoints, and project/developer scope.
- Default OpenRouter dependency as the only path. AMP should keep local embeddings viable.
- Direct code transfer before adapting it to AMP's project/developer scope, provenance rules, and storage contracts.
...
- Palace/wings/rooms/drawers as the public AMP model. Useful as inspiration, but AMP should keep neutral project/session/event/chunk terms.
- ChromaDB as SoT.
...
- WAL/adapter lessons for SQLite if AMP ever supports edge/local single-user mode.

What not to take directly:

- Product-specific SQLite schema as AMP core.
- UI modes (`Intro`, `Access`, `Rules`) as required AMP concepts.
- Any code path that bakes MF0 product-specific modes into AMP core instead of treating them as optional UX/pipeline ideas.
...
- Do not base AMP on the sunset OpenMemory package.
- Do not confuse it with CaviraOSS/OpenMemory in docs.

## Journey / Journey Kits by Matthew Berman / Forward Future

Current positioning: a free/open registry for reusable agent workflows. Its public site describes Journey as a place to install kits and give agents new capabilities. It is relevant to AMP because the owner needs "new project -> one action -> usable agent configuration" without manually copying project rules and logs.

Architecture decision after comparison with OB1: Journey should not replace OB1 as AMP's memory foundation. Journey is the reference for packaging/onboarding/workflow distribution; OB1 remains the preferred memory-substrate backbone. See ADR-0008.
...
- `kit.md`-like packaging for AMP project setup or reusable memory workflows.
...
- Learning/outcome feedback loop as an optional private local pattern for improving AMP templates.
- Shared resource binding concepts for server URLs, DBs, credentials, and future organization/team contexts.

What not to take directly:

- Do not make AMP core memory depend on Journey SaaS availability.
...
This is the owner's prior personal project started before AMP. It should be treated as an idea sketch / first pass at the same problem AMP now addresses more seriously, not as a mature external upstream implementation.
...
- Repo-native fallback surfaces that AMP can generate during `amp init`.
...
- Do not bake the owner's current server layout into AMP just because the early sketch assumed a local workflow.
...
- AMP should keep L0 append-only events as SoT, then derive chunks, embeddings, facts, summaries, and traces.

### 2. Memory write-back must be governed

OB1 Agent Memory is the best current source for this.

AMP should distinguish:
...
AMP should probably start with relations between chunks/events/memories and temporal facts, not a full visual graph product.
...
- AMP already has developer/project scope; keep it central.
...
MemPalace shows what a powerful large tool surface looks like, but AMP should not expose 29 tools in v1.
...
6. **OB1/MF0 synthesis** is accepted: OB1 supplies governance/trust; MF0 supplies workbench/raw-capture/Memory Tree/Keeper ideas; AMP owns capture profiles and Review UI policy.
...
2. Refine the AMP core memory taxonomy: raw events, chunks, governed memories, temporal facts, checkpoints.
