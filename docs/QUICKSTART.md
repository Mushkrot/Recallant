...
onboarding. To switch to the moving development channel, explicitly reinstall with `--ref main`.
...
```bash
recallant --version
# recallant 0.1.0-dev.3+<git-sha>
```

`recallant --version` reports the shared release version with git build metadata when installed from
a checkout. It should report `0.1.0-dev.3` plus the exact source revision for this prerelease.
...
`recallant onboard` is the universal beginner command for local machines and remote clients. It uses
reachable local storage or a central server supplied through project configuration, deployment
environment, or `--server-url`. If neither route is configured, it stops before changing project
files and shows both setup paths:
...
`recallant connect` remains an advanced compatibility alias:

```bash
recallant connect /path/to/project
```

In an interactive terminal, onboarding first checks the installed CLI revision against the official
`Mushkrot/Recallant` `main` branch. If a newer revision is available, Recallant shows the current and
latest revisions and asks `Update Recallant now? [Y/n]`. Accepting installs the update from the
official HTTPS repository and resumes the same onboarding command automatically. If the repository
cannot be reached, onboarding continues normally. Use `--no-update-check` to skip this preflight.

This moving-channel check applies to a `main` installation. A tagged prerelease stays pinned to its
tag, and onboarding reports that the `main` development channel was not selected. Changing from a
tagged release to `main` requires an explicit bootstrap reinstall with `--ref main`.

An installation that predates the update check needs one final manual refresh. From a Recallant
checkout, pull the current `main` branch and rerun `./scripts/install-recallant-cli.sh --user`; a
workstation without a checkout can rerun its central server's `curl .../connect | bash` bootstrap.
Refreshing the CLI does not recreate project files or local storage.

The one-command flow selects only a reachable local route or an explicitly configured remote route,
