...
Recallant. It removes only Recallant local pointer/runtime files such as `.recallant/config`,
`.recallant/codex-mcp.json`, `.recallant/generic-mcp.json`, and `.recallant/current-session.json`;
`AGENTS.md`, `PROJECT_LOG.md`, source files, and attach backups remain in place.
...
6. writes and prints MCP config blocks for the target client;
...
`--target codex` is the near-term default path because the owner's current workflow is Codex-first.
`--target generic` writes `.recallant/generic-mcp.json` for compatible MCP clients. Other targets
remain supported by architecture: `cursor`, `claude-code`, `windsurf`, and `generic`.
...
Use the block generated by `recallant attach` or `recallant init`.

Client-specific startup notes with official links are in [CLIENT_SETUP.md](CLIENT_SETUP.md). The
short version:

- Codex: use `.recallant/codex-mcp.json` or `codex mcp add` with the same command/env values.
- Cursor: copy `.recallant/generic-mcp.json` into `.cursor/mcp.json`.
- Claude Code: run `claude mcp add --transport stdio ... recallant -- recallant mcp-server`.
- Windsurf: copy `.recallant/generic-mcp.json` into `~/.codeium/windsurf/mcp_config.json`.

After that, the client connects to Recallant the next time the folder is opened or the MCP config is
reloaded.
