...
- use reachable local storage or a central server supplied through project configuration,
  deployment environment, or `--server-url`;
- stop before changing project files when neither route is configured;
...
capture fallback, but the universal command may select only a reachable local path or an explicitly
configured central path. Otherwise it must return the setup choices without project writes.
...
For an empty project, `recallant onboard <project>` can now create starter docs through the confirmed
...
Remote connect uses the same thin-file shape. `recallant onboard <project>` and the advanced
`recallant connect-cloud <project> --server-url <https-url>` generate or safely upsert
