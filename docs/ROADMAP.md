...
- Remote project access to a central Recallant server: authenticated `POST /api/mcp`, scoped remote
  MCP credential lifecycle, one-time invite onboarding, CLI/Workbench provisioning output, and the
  ...
  `remote-mcp-external-rehearsal:smoke`; remaining near-term work is repeat real separate-machine
  rehearsal using the one-command invite plus the `recallant remote-acceptance` redacted evidence
  gate, broader onboarding, and broader client transport support.
