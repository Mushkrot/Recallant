...
- Remote project access to a central Recallant server: the first authenticated `POST /api/mcp`
  ...
  rehearsal using the `recallant remote-acceptance` redacted evidence gate with operator-provided
  live endpoint/credential/scope, broader onboarding, and broader client transport support before
  remote access becomes the default client path.
