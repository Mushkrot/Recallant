...
- Remote project access to a central Recallant server: design and implement authenticated remote
  agent/MCP access so projects on another server or workstation can attach to one managed Recallant
  instance without exposing Postgres, raw artifacts, backups, or unauthenticated MCP/admin routes.
...
- Remote-client onboarding packages and connection diagnostics for projects that use a central
  Recallant server from another machine.
