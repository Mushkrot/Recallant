...
| Cross-project data leak | Каждый query фильтрует `project_id`; server process bound to single `AMP_PROJECT_ID` (рекомендация v1). |
...
| Accidental public exposure of management UI | Review UI is private by default. Future Cloudflare/subdomain access must require explicit owner configuration, Cloudflare Access or equivalent edge auth, AMP auth/session policy, and no unauthenticated public admin surface. |
...
| Backup exposure | Backups are encrypted before leaving AMP server; manifests must not contain raw secrets; future second-server replication uses private SSH/Tailscale or an explicit secure transport. |
...
- AMP Review UI/admin API require AMP-level auth even on Tailnet/SSH routes. Private network access alone is not enough.
...
- Cloudflare deployment requires edge auth such as Cloudflare Access or equivalent plus AMP auth/session. Cloudflare alone does not replace AMP auth.
...
```text
Tailscale/SSH -> AMP auth -> Review UI/admin API
```

Cloudflare-ready future mode:

```text
Cloudflare-managed subdomain -> edge auth -> AMP auth -> Review UI/admin API
```
