...
- Phase 6.5 Review UI is implemented as a private server-rendered Review Command Center on the Recallant server package. It requires Recallant bearer/session/Cloudflare-ready auth for `/review` and `/api/review-dashboard`, binds through the HTTP server default private host path, and shows project list, critical status, Review Inbox, import candidates, conflicts/duplicates, Active Rules, selected detail/source refs/history/actions, Cost / Paid API, Settings, cleanup/forget, and a functional management chat. The UI/API smoke covers Russian chat answers, confirmation-gated destructive/paid/public/global-risk requests, dangerous setting gates, secret-setting redaction, review actions, detach, and forget-forever forms. It is verified by `scripts/smoke-review-ui.mjs`.
