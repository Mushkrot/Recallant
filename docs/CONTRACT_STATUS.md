...
| Memory-loop and capture proof | Working slice with freshness semantics | Onboard verify reports structured memory-loop/readiness/recall stages; readiness v2 keeps `memory_loop_ready` separate from `capture_active`; `capture_active` requires a fresh automatic Codex hook event and becomes stale after the configured window; Codex owners must review and trust the project command hook through `/hooks` before that event can arrive; `npm run capture-readiness:smoke`, `npm run product-acceptance:smoke`, `npm run agent-capture:smoke`, and `npm run agent-lifecycle-gate:smoke` cover the governed workflow and degraded paths | Broader native-client freshness pilots before release candidate. |
