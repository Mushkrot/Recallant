...
| Release identity, CLI version, and onboarding update channel | Working prerelease slice | Root and private workspace manifests agree on `0.1.0-dev.0`; runtime MCP/CLI surfaces use the same identity plus optional exact Git revision. Interactive `recallant onboard` checks the official public `main` revision only for the moving `main` channel, renders a confirmation panel when an update exists, refuses dirty or custom checkouts, and fails open when the repository is unavailable. A tagged `v0.1.0-dev.0` install stays pinned and never silently switches to `main`; switching channels requires an explicit bootstrap reinstall. `npm run release-identity:smoke`, `npm run release-install:smoke`, `npm run cli-version:smoke`, `npm run cli-update:smoke`, and `npm run public-quickstart:smoke` cover the contract. | npm publication and stable-version compatibility guarantees remain out of scope. |
