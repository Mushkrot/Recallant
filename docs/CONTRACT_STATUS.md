...
| Agent run observability and replay | Working slice | Project-scoped `agent_observations` correlation envelope, deterministic session/run ordering, deduplication, redaction, retention, MCP `memory_append_observation`, CLI `recallant agent-observe`, fail-soft project hook kit, compatible lifecycle event derivation, Workbench Activity **Runs**, **Replay**, **Errors**, and **Coverage** tabs, evidence-based completeness, targeted forget redaction, project purge, and native backup/restore; `npm run agent-observability:smoke`, `npm run review-ui:smoke`, `npm run review-ui:playwright`, `npm run project-sanitize:smoke`, `npm run phase8:smoke:backup` | Broader external client adapter pilots and optional monitoring/export integrations remain future work. |
