...
| Capture-active proof | Working slice with quickstart proof | `doctor --require-capture` distinguishes configured from capture active; `public-quickstart:smoke` proves installed-wrapper onboarding, `doctor --require-capture`, and `ask`; `npm run product-acceptance:smoke`, `npm run demo-capture:smoke`, `npm run agent-capture:smoke` | Keep proof visible in Workbench and docs. |
...
sandbox copies, Workbench migration review ergonomics, autonomous browser QA with synthetic
screenshots, pilot report scenarios, and production-sensitive dry-run safety, plus
