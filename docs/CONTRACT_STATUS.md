...
| Agent run observability and replay | Working slice with automatic native Codex adapter | Project-scoped `agent_observations` correlation envelope, deterministic session/run/tool/subagent ordering, deduplication, bounded redaction, retention, MCP `memory_append_observation`, CLI `recallant agent-observe`, silent fail-soft `recallant codex-hook`, structural `.codex/hooks.json` merge/backup, truthful configured-unobserved/observed readiness, offline spool, compatibility hook kit, Workbench Activity **Runs**, **Replay**, **Errors**, and **Coverage** tabs, evidence-based completeness, targeted forget redaction, project purge, and native backup/restore; `npm run codex-hook-adapter:smoke`, `npm run codex-hook-runtime:smoke`, `npm run connect:smoke`, `npm run agent-observability:smoke`, `npm run review-ui:smoke`, `npm run review-ui:playwright`, `npm run project-sanitize:smoke`, `npm run phase8:smoke:backup` | Native adapters for more clients, hook-invisible hosted-tool coverage, broader external client pilots, and optional monitoring/export integrations remain future work. |
...
embeddings, and idempotent `connect codex --dry-run` with `writes_files: false`.
...
- Project purge is not a substitute for source repository cleanup. It removes Recallant-controlled
  project memory and agent observations and disconnects Recallant-generated local artifacts,
