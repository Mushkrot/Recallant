...
| Memory-loop and capture proof | Working slice with freshness semantics | Onboard verify reports structured memory-loop/readiness/recall stages; readiness v2 keeps `memory_loop_ready` separate from `capture_active`; `capture_active` requires a fresh automatic Codex hook event and becomes stale after the configured window; `npm run capture-readiness:smoke`, `npm run product-acceptance:smoke`, `npm run agent-capture:smoke`, and `npm run agent-lifecycle-gate:smoke` cover the governed workflow and degraded paths | Broader native-client freshness pilots before release candidate. |
...
| Release identity, CLI version, and onboarding update channel | Working prerelease slice | Root and private workspace manifests agree on `0.1.0-dev.1`; runtime MCP/CLI surfaces use the same identity plus optional exact Git revision. Interactive `recallant onboard` checks the official public `main` revision only for the moving `main` channel, renders a confirmation panel when an update exists, refuses dirty or custom checkouts, and fails open when the repository is unavailable. A tagged `v0.1.0-dev.1` install stays pinned and never silently switches to `main`; switching channels requires an explicit bootstrap reinstall. `npm run release-identity:smoke`, `npm run release-install:smoke`, `npm run cli-version:smoke`, `npm run cli-update:smoke`, and `npm run public-quickstart:smoke` cover the contract. | npm publication and stable-version compatibility guarantees remain out of scope. |
