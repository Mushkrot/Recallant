...
coverage, but may still need broader client pilots, UI polish, repeat rehearsals, or
...
| Memory-loop and capture proof | Working slice with freshness semantics | Onboard verify reports structured memory-loop/readiness/recall stages; readiness v2 keeps `memory_loop_ready` separate from `capture_active`; `capture_active` requires a fresh automatic Codex hook event and becomes stale after the configured window; Codex owners must approve the project command hook through the actual client's trust surface before that event can arrive; Codex Desktop does not guarantee a `/hooks` slash command; `npm run capture-readiness:smoke`, `npm run product-acceptance:smoke`, `npm run agent-capture:smoke`, and `npm run agent-lifecycle-gate:smoke` cover the governed workflow and degraded paths | Broader native-client freshness pilots before release candidate. |
| Cross-Client Continuity | Shared-store and generic MCP foundation only; defining product proof not delivered | Codex has native automatic capture; Claude Code, Cursor, Windsurf, and generic clients can receive MCP configuration; `npm run cross-client:smoke` proves two generic MCP processes can use one store | Add a native Claude Code lifecycle adapter and a real Codex -> Claude Code -> Codex acceptance journey with no pasted recap, including restart, remote, same-name isolation, spool/recovery, and deletion checks. Generic subprocess labels do not satisfy this gate. |
| Human Document Memory | Planned after Cross-Client Continuity; not delivered | Existing source records, chunking, embeddings, hybrid retrieval, provenance, Workbench ask/review, vault inventory/candidate/export, backup, and erasure are reusable foundations | Add explicit folder attachment for Markdown, text, PDF, and DOCX; stable source identity/versioning; incremental add/update/rename/delete; extraction boundaries; source-cited answers; and human/agent query parity. No broad filesystem crawl is implied. |
| Task Handoff Record | Planned after Human Document Memory; not delivered | Existing sessions, checkpoints, closeout, governed memories, source refs, review, and audit can support an evidence-linked work-state layer | Define a Recallant-owned claim/pause/resume/finish record and receipt. Linear is not a dependency. Plane or another task manager may only be an optional adapter or board, not the memory authority. |
...
| Scoped content erasure | Working slice | Current-project direct event/chunk/memory/artifact targets, related agent-observation redaction, bounded search and exact scope selectors, no-write previews, content-free selection digests and receipts, stale-token rejection, transactional dependent cleanup, external-object boundary warnings, and Workbench confirmation UX; `npm run phase6:smoke:graph`, `npm run agent-observability:smoke`, `npm run review-ui:smoke`, `npm run build`, `npm run lint`, `npm run public-readiness:smoke`, `npm run public-security:smoke` | Broader real-project erasure pilots before release candidate. |
| Agent run observability and independent control | Working slice with native Codex adapter and OTel control | Project-scoped `agent_observations`, automatic error → retry → remediation → verification/regression chains, deterministic confidence/reasons, bounded redaction/retention, native `recallant codex-hook`, content-free scoped OTLP/HTTP JSON ingestion, OTel/native reconciliation, truthful capture freshness, offline spool, Workbench Activity **Runs**, **Replay**, **Errors**, and **Coverage**, targeted forget, project purge, and native backup/restore; `npm run codex-hook-adapter:smoke`, `npm run codex-hook-runtime:smoke`, `npm run codex-otel-contract:smoke`, `npm run codex-otel-control:smoke`, `npm run capture-readiness:smoke`, `npm run agent-observability:smoke`, `npm run review-ui:smoke`, `npm run review-ui:playwright`, `npm run project-sanitize:smoke`, `npm run phase8:smoke:backup` | Native adapters for more clients, hook-invisible hosted-tool coverage, and broader external-client OTel pilots remain future work. |
...
| Release identity, CLI version, and onboarding update channel | Working prerelease slice | Root and private workspace manifests agree on `0.1.0-dev.3`; runtime MCP/CLI surfaces use the same identity plus optional exact Git revision. Interactive `recallant onboard` checks the official public `main` revision only for the moving `main` channel, renders a confirmation panel when an update exists, refuses dirty or custom checkouts, and fails open when the repository is unavailable. A tagged `v0.1.0-dev.3` install stays pinned and never silently switches to `main`; switching channels requires an explicit bootstrap reinstall. `npm run release-identity:smoke`, `npm run release-install:smoke`, `npm run cli-version:smoke`, `npm run cli-update:smoke`, and `npm run public-quickstart:smoke` cover the contract. | npm publication and stable-version compatibility guarantees remain out of scope. |
...
| Public OSS surface | Working prerelease slice | Public docs boundary, reproducible 1440 x 900 Workbench portfolio screenshots with synthetic data, exact screenshot-set checks, versioned release notes, pinned installer contract, readiness smoke, and forbidden private marker checks across docs and public code; `npm run portfolio-screenshots:generate`, `npm run release-identity:smoke`, `npm run release-install:smoke`, `npm run public-readiness:smoke`, `npm run public-security:smoke` | Stable-release packaging and independent presentation review remain future work. |
...
workspace reported `agent-start` mode `server`, the then-current v1 working-loop readiness proof,
client connection `mcp_and_hooks_ready`, local hook readiness, zero pending embeddings, and
idempotent `connect codex --dry-run` with `writes_files: false`. Readiness v2 now names that evidence
`memory_loop_ready`; current `capture_active` is reserved for fresh automatic telemetry.
...
- Browser-first project attachment from the Workbench is future work. The current beginner MVP is a
  universal CLI flow where the user runs one command: `recallant onboard <project>`.
...
- `attach`, client-specific `connect`, `doctor`, and `agent-*` are still useful advanced/debug APIs,
  but the public beginner path should not require users to know those commands.
...
- Project purge is not a substitute for source repository cleanup. It removes Recallant-controlled
  project memory, agent observations, and independent OTel control records and disconnects Recallant-generated local artifacts,
  including generated hooks and offline spool files; it must not delete source files, secrets,
  downloads, or arbitrary project data. System activity rows are retained only as de-identified
  governance evidence after purge, not as a project recall source.
...
- Agent activity replay reflects the bounded evidence a client reported. Completeness checks expose
  internal gaps but cannot prove that an external client reported every action it performed.
...
- Broader personal-life memory, team/multi-user workflows, and richer connector ecosystems remain
  future expansion after real cross-client continuity and the bounded Human Document Memory MVP.
...
- a real Codex -> Claude Code -> Codex continuation gate with native lifecycle capture on both
  clients and no pasted human recap;
