...
| Governed graph tree contract | Working B2 storage/review and Markdown vault bridge slice with typed public vocabulary; runtime remains compatible with the current bounded edge graph | `docs/GRAPH_TREE_CONTRACT.md` defines graph node kinds, relation types, lifecycle states, candidate lifecycle, governance requirements, and the dry-run/default vault bridge contract; `packages/contracts` exports the typed graph vocabulary; `graph_candidates`, `graph_candidate_source_refs`, and `graph_candidate_review_actions` store project-scoped staged proposals; MCP exposes `memory_create_graph_candidate`, `memory_list_graph_candidates`, `memory_get_graph_candidate`, and `memory_review_graph_candidate`; CLI exposes `recallant vault inventory`, `recallant vault candidates`, and `recallant vault export`; current graph-expanded search remains edge-based and candidates are not retrieval-active by default; `npm run graph-candidates:smoke`, `npm run vault-bridge:smoke`, `npm run phase6:smoke:graph`, `npm run mcp:smoke`, `npm run build`, `npm run format:check`, `npm run lint`, `npm run public-readiness:smoke`, `npm run public-security:smoke` | Keeper automation, graph retrieval profiles, first-class graph nodes, graph storage migration decisions, promotion from accepted candidates into graph state, passive vault sync, raw media ingestion, and Workbench topology/review views remain future work. |
