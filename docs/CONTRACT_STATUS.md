...
| Context-pack startup and closeout | Working slice with lifecycle readiness gate | MCP/CLI startup, checkpoint, closeout, recovery, and local spool paths; `npm run phase3:smoke`, `npm run spool:smoke`, `npm run agent-lifecycle-gate:smoke`, `npm run product-acceptance:smoke` | Improve closeout extraction and reporting quality. |
