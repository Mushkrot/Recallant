...
AMP must not depend on one model or one provider for all memory work. The owner has a Linux server with local compute/GPU capacity, but also wants access to stronger non-local reasoning for tasks where quality matters.
...
Design AMP with a **local-first model router**.
...
If subscription-backed routes hit limits, AMP must pause, defer, downgrade to local models, or create an explicit approval request before using paid API. It must not silently convert subscription exhaustion into paid API spend.
...
- AMP remains useful offline or when cloud APIs fail.
- Subscription-backed and paid API usage can be audited, disabled, budgeted, and reasoned about separately.
- Model routing is part of AMP's memory runtime, not a generic all-purpose LLM gateway.
...
This ADR does not turn AMP into a universal provider gateway for unrelated applications. Routing is for AMP memory tasks: embedding, retrieval/rerank, extraction, consolidation, closeout, review assistance, cleanup, and similar memory lifecycle operations.
