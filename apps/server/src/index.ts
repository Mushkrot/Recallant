...
function publicHttpErrorMessage(error: unknown) {
  switch (httpAuditErrorCode(error)) {
...
async function dispatchRemoteMcpJsonRpc(input: {
  ...
  if (method === "initialize") {
    return remoteMcpJsonRpcResult(id, {
      ...
      serverInfo: { name: recallantMcpServerName, version: recallantContractVersion },
...
function formatDate(value: unknown) {
  if (!value) return "";
  if (publicScreenshotMode()) return "Recent";
...
async function withAuditReport(
  ...
  try {
    ...
    return {
      ...dashboard,
      audit_error: publicHttpErrorMessage(error)
    };
  }
}
...
async function withSearchResults(
  ...
  try {
    ...
    return {
      ...
      search_results: {
        memories: [],
        error: publicHttpErrorMessage(error)
      }
    };
  }
}
