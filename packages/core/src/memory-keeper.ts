...
import { containsPrivateKeyBlock, redactPrivateKeyBlocks } from "./private-key-redaction.js";
...
export function redactKeeperText(text: string) {
  return redactPrivateKeyBlocks(text, "[redacted_private_key]")
...
export function keeperProposalId(parts: readonly string[]) {
  return parts
    .map((part) => trimRepeatedBoundary(part.toLowerCase().replace(/[^a-z0-9]+/g, "-"), "-"))
    .filter(Boolean)
    .join(":");
}

function trimRepeatedBoundary(value: string, boundary: string) {
  let start = 0;
  let end = value.length;
  while (start < end && value[start] === boundary) start += 1;
  while (end > start && value[end - 1] === boundary) end -= 1;
