...
function slug(value: string, fallback: string) {
  const separated = value.toLowerCase().replace(/[^a-z0-9]+/g, "_");
  let start = 0;
  let end = separated.length;
  while (start < end && separated[start] === "_") start += 1;
  while (end > start && separated[end - 1] === "_") end -= 1;
  const normalized = separated.slice(start, end);
